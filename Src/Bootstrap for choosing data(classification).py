...
def baseline_model(shape):
    # create model
    model = keras.Sequential()
    model.add(keras.layers.Dense(128, input_dim=shape, activation='relu'))
    ...
    # Compile model
    # .RMSprop(0.001)
    model.compile(loss='categorical_crossentropy', optimizer=keras.optimizers.Adamax(0.005), metrics=['accuracy'])
    return model
...
train = ut.resample(train, replace=True, n_samples=5000, random_state=1)
test = ut.resample(test, replace=True, n_samples=800, random_state=1)
...
model = baseline_model(train_dataset.shape[1])
hist = model.fit(train_dataset, dummy_y_train, batch_size=64, epochs=1000, validation_split=0.2)
# evaluate model
